...
export default function CreatePage() {
  ...
  if (state) {
    ...
    for (let i = 0; i < state["recipeDetail"].recipe_order.length; i++) {
      ...
      if (recipe_img) {recipe_order_img[order] = recipe_img;}
    }
  }
    ...
    <div>
      {state ? (
        <CreateForm
          ...
          show_recipe_thumbnail={(!state["recipeDetail"].api_recipe) && state["recipeDetail"].recipe_thumbnail}
          title={state["recipeDetail"].title}
          description={state["recipeDetail"].description}
          recipe_ingredients={recipe_ingredients_str}
          recipe_order_content={(recipe_order_content.length > 0) && recipe_order_content}
